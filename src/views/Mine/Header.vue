<template>
    <div class="header">
        <div class="user">
            <div class="img">
                <img src="https://m.mi.com/static/img/avatar.76a75b8f17.png">
            </div>            
            <span>登录/注册</span>
        </div>  
        <div class="order">
            <mt-cell
                title="我的订单"
                to="//github.com"
                is-link
                value="全部订单">
            </mt-cell>
        </div>   
        <div class="items">
            
                <div class="item">
                    <img src="data:image/png;base64,">
                    <span>待付款</span>
                </div>
                <div class="item">
                    <img src="data:image/png;base64,">
                    <span>待收货</span>
                </div>
                <div class="item">
                    <img src="https://m.mi.com/static/img/nav-4.d68723895f.png">
                    <span>退换修</span>
                </div>
               
        </div>
         <div class="vip" >
             <div >
                 <div class="img">
                    <img slot="icon" src="data:image/png;base64," width="26" height="26">
                </div>
                <mt-cell
                    title="会员中心"
                    to=""
                    is-link
                    value="">
                </mt-cell>
            </div>
            <div >
                 <div class="img">
                    <img slot="icon" src="data:image/png;base64," width="26" height="26">
                </div>
                <mt-cell
                    title="我的优惠"
                    to=""
                    is-link
                    value="">
                </mt-cell>
            </div>

             
            
        </div>
          <div class="vip" >
             <div >
                 <div class="img">
                     
                    <img slot="icon" src="data:image/png;base64," width="26" height="26">
                </div>
                <mt-cell
                    title="服务中心"
                    to=""
                    is-link
                    value="">
                </mt-cell>
            </div>
            <div >
                 <div class="img">
                    <img slot="icon" src="data:image/png;base64," width="26" height="26">
                </div>
                <mt-cell
                    title="小米之家"
                    to=""
                    is-link
                    value="">
                </mt-cell>
            </div>

             
            
        </div>
         <div class="vip" >
             <div >
                 <div class="img">
                     
                    <img slot="icon" src="data:image/png;base64," width="26" height="26">
                </div>
                <mt-cell
                    title="F码通道"
                    to=""
                    is-link
                    value="">
                </mt-cell>
            </div>
            
        </div>
        <div class="vip" >
             <div >
                 <div class="img">
                     
                    <img slot="icon" src="https://m.mi.com/static/img/i-setting.fb9625b3f2.png" width="26" height="26">
                </div>
                <mt-cell
                    title="设置"
                    to=""
                    is-link
                    value="">
                </mt-cell>
            </div>
            
        </div>
               
    </div>
</template>

<script>
export default {

}
</script>

<style lang="scss">
    .header{
      
        
        .user{
            display: flex;
            align-items: center;
            justify-content: flex-start;
            width: 100%;
            
            // line-height: 0.9rem;
            height: 0.9rem;
            padding: 0.18rem;
            background: url("https://m.mi.com/static/img/bg.63c8e19851.png" ) center #f37d0f;
            background-size: 100%;
            .img{
                float: left;
                width:0.5rem;
                height: 0.5rem;
                overflow: hidden;
                box-sizing: border-box;
                border-radius: 100%;
                text-align: center;
                border: 3px solid hsla(0, 0%, 100%, .4);
                img{
                    width: 100%;
                }
            }
            span{
                color:#fff;
                font-size: 14px;
                margin-left: 10px;
            }
        }
        .mint-cell-value{
            font-size: 14px;
        }
        .items{
            width: 100%;
            height: 0.9rem;
            display: flex;
            justify-content: space-around;
            align-items: center;
             background: #fff;
            .item{
                width: 0.36rem;
                display: flex;
                flex-direction: column;
                align-items: center;
                justify-content: center;
                img{
                    width: 25px;
                }
                span{
                    margin-top: 5px;
                    font-size: 12px;
                }
            }
        }
        .vip{
            margin-top:15px;
            .img{
                background: #fff;
                width: 48px;
                height: 48px;
                float: left;
                display: flex;
                justify-content: center;
                align-items: center;
                img{
                    float: left;
                
                }
            }
            .mint-cell-wrapper{
                background: none;
            }
         
            
           
        }
    }
</style>
